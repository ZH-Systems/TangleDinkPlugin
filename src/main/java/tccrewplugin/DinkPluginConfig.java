...
public interface DinkPluginConfig extends Config {
    ...
    @ConfigItem(
        keyName = "remoteEventConfigUrl",
        name = "Remote Event Config URL",
        description = "Remote JSON endpoint polled for clan event activation prompts.",
        position = -30,
        section = clanEventSection
    )
    default String remoteEventConfigUrl() {
        return "https://raw.githubusercontent.com/pajlads/DinkPlugin/master/migrations/remote-event-config.json";
    }

    @ConfigItem(
        keyName = "remoteEventPollEnabled",
        name = "Poll Remote Events",
        description = "Whether Dink should periodically check for remotely announced clan events.",
        position = -29,
        section = clanEventSection
    )
    default boolean remoteEventPollEnabled() {
        return true;
    }

    @ConfigItem(
        keyName = "remoteEventPollIntervalSeconds",
        name = "Remote Poll Interval",
        description = "How often Dink should poll the remote event config endpoint.",
        position = -28,
        section = clanEventSection
    )
    @Units(Units.SECONDS)
    @Range(min = 10)
    default int remoteEventPollIntervalSeconds() {
        return 60;
    }

    @ConfigItem(
        keyName = "allowedMigrationHosts",
        name = "Allowed Migration Hosts",
        description = "Allowlist of hosts permitted for remote event migration payloads.<br/>" +
            "Separate multiple hosts with commas, semicolons, or new lines.",
        position = -27,
        section = clanEventSection
    )
    default String allowedMigrationHosts() {
        return "raw.githubusercontent.com";
    }

    @ConfigItem(
        keyName = "remoteEventLastSeenVersion",
        name = "",
        description = "",
        hidden = true
    )
    default int remoteEventLastSeenVersion() {
        return 0;
    }
    ...
    void setRemoteEventLastSeenVersion(int version);

    @ConfigItem(
        keyName = "remoteEventLastPromptedEventId",
        name = "",
        description = "",
        hidden = true
    )
    default String remoteEventLastPromptedEventId() {
        return "";
    }
    ...
    void setRemoteEventLastPromptedEventId(String eventId);

    @ConfigItem(
        keyName = "remoteEventAppliedEventIds",
        name = "",
        description = "",
        hidden = true
    )
    default String remoteEventAppliedEventIds() {
        return "";
    }
    ...
    void setRemoteEventAppliedEventIds(String eventIds);
