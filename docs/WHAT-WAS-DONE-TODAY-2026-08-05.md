...
- Fixed the RuneLite LFG leave-group flow so the sidebar and `::lfgleave` action trust backend viewer permissions instead of guessing membership from the locally displayed RSN.
- Added a regression test covering leave-button rendering when the player can leave a group but their current local display name does not string-match the member row.
